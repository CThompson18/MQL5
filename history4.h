...
extern string File_Name="historynew";
extern string StartDate="2015.04.15 00:00";

// Dan's TrendSignal settings
extern int Signal_RISK=15;
		...
		for (int kk=CountBars-1;kk>=0;kk--)
			{
				...
				if (dan_up != 0 && (last_s!=1 || (tp[kk+1]==EMPTY_VALUE && pb[kk+1]==EMPTY_VALUE)))
					{
						...
						if (tp[kk+1]!=EMPTY_VALUE) 
							{
								SUM+=(MathAbs(last_p-tp[kk+1])/Point)/mult;
								trades ++;
				...
				if (High[kk]>=tp[kk] && Low[kk]<tp[kk]) 
					{
						SUM+=(MathAbs(last_p-tp[kk])/Point)/mult;
						trades ++;
			  ...
			  if (High[kk]>=sl[kk] && Low[kk]<=sl[kk]) 
				{
					SUM+=(MathAbs(last_p-sl[kk])/Point)/mult;
					trades ++;
