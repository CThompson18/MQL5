...
extern double part1_up_level = 9.5;
extern double part1_down_level = -9.5;
extern double part2_up_level = 6;
extern double part2_down_level = -6;
extern double part3_up_level = 9;
extern double part3_down_level = -9;
...
int start()
{
   if (TotalOrders(magic_number) == 0 && TotalOrdersInLastHour(magic_number) < max_trades_per_hour)
   {
      ...
      Comment("\nPBT: "+DoubleToStr(pbt, 1)+"\nScalping Trend: "+DoubleToStr(scalping_trend, 1)+"\nATR Speed 1: "+DoubleToStr(atr_speed_1, 1)+"\nATR Speed 2: "+DoubleToStr(atr_speed_2, 1)+"\nGlobal Value: "+DoubleToStr(global_trend_value, 1));
      
      if ((pbt >= part1_up_level && global_trend_value >=0 && global_trend_value <part2_up_level) ||
          (pbt >= part1_up_level && global_trend_value >= part2_up_level && (atr_speed_1+atr_speed_2+scalping_trend)>= part3_up_level))
      {
      ...
      if ((pbt <= part1_down_level && global_trend_value <=0 && global_trend_value > part2_down_level) ||
         (pbt <= part1_down_level && global_trend_value <= part2_down_level && (atr_speed_1+atr_speed_2+scalping_trend)<= part3_down_level))
      {
