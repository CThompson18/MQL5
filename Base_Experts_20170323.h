...
extern string glbStartDate = "2017.03.01 00:00";
            							...
            							{ 
            								...
            								blnBid = False;										   
      									   ...
      									   int CountBars = iBarShift(Symbol(),0,StrToTime(glbStartDate),false);
			                                    ...
			                                    {
      									               double dblBid = iCustom(Symbol(),Signal_Timeframe,"Base",glbRisk,CountBars,1,kk+1);
      									            }
      										glbCurrent_Bid = SymbolInfoDouble(Symbol(),SYMBOL_BID);
      										if (glbCurrent_Bid < glbPrice_Bid)
      										   {                                     
            										//CloseAll();
            										//Order_Bid();							
            										//DataDisplay("Bid",glbPrice_Bid,"Current_Bid",glbCurrent_Bid);
            										Comment(StringFormat("Show prices\nBidPrice = %G\nCurrentBid = %G\niCustomBid = %G\nAskPrice = %d",glbPrice_Bid,glbCurrent_Bid,dblBid,glbPrice_Ask));
...
/*      									   int CountBars = iBarShift(Symbol(),0,StrToTime(glbStartDate),false);
      									   		for (int kk = CountBars - 1;kk >= 0; kk--)
			                                    {
      									               double dblAsk = iCustom(Symbol(),Signal_Timeframe,"Base",glbRisk,CountBars,0,kk+1);
      									            }
*/      									
   										glbCurrent_Ask = SymbolInfoDouble(Symbol(),SYMBOL_ASK);
   										if (glbCurrent_Ask < glbPrice_Ask)
   										   { 																						
         										//CloseAll();
         										//Order_Ask();
         										//DataDisplay("Ask",glbPrice_Ask,"Current_Ask",glbCurrent_Ask);
         										Comment(StringFormat("Show prices\nAskPrice = %G\nCurrentAsk = %G\nBidPrice = %d",glbPrice_Ask,glbCurrent_Ask,0));
