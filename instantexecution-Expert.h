            ...
            if(TrailingStart>0)
              {
               if((OrderOpenPrice()-Ask)>(TrailingStart*_pip))
                 {
                  if((OrderStopLoss()>(Ask+TrailingStart*_pip)) || (OrderStopLoss()==0))
                    {
