	   ...
	   while(shift>=0)
		 {
		   ...
		   Counter=shift;
			...
			for (Counter=shift; Counter<=shift+9; Counter++) AvgRange=AvgRange+MathAbs(High[Counter]-Low[Counter]);
				...
				Counter=shift;
				TrueCount=0;
				Print("for Counter=Shift ----- shift=",shift, "    Counter=",Counter, "    AvgRange=",AvgRange, "    TrueCount=",TrueCount);
			while (Counter<shift+9 && TrueCount<1)
				{
					if (MathAbs(Open[Counter]-Close[Counter+1])>=Range*2.0) TrueCount=TrueCount+1;
					Print("While Counter<Shift+9 ----- shift=",shift, "    Counter=",Counter, "    TrueCount=",TrueCount);
					Counter=Counter+1;
				}
			if (TrueCount>=1) {MRO1=Counter;} else {MRO1=-1;}
				Counter=shift;
				Print("If TrueCount>=1 ----- shift=",shift, "    Counter=",Counter, "    TrueCount=",TrueCount,"    MRO1=",MRO1);
				TrueCount=0;
