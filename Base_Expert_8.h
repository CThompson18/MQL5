...
void OnTick()
   {
      ...
      Prices();
      DataDisplay();
      if (glbBid >= 0){glbCurrentAskBid = 1;}
      if (glbAsk >= 0){glbCurrentAskBid = 2;}
      ...
      for (cnt = 0 ; cnt <=intOrders ; cnt++)
		   {
				...
				Comment(StringFormat("Show ticket\nTicket = %d",intTicket,0));
            if (OrderType()==OP_BUY){intAskBid = 1;}
            if (OrderType()==OP_SELL){intAskBid = 2;}
			}

      if(intOrders != 0){if (intAskBid == 2){if(glbCurrentAskBid == 2){CloseAll();Order_Bid();}}}
      if(intOrders != 0){if (intAskBid == 1){if(glbCurrentAskBid == 1){CloseAll();Order_Ask();}}}
...
void DataDisplay()
   {
		...
		text = "***************************************************";
		createObject(13,OBJ_LABEL,0,xAxis,yAxis + glb_upper_edge_shift * 13,text);
		
		text= "CurrentAskBid = "+DoubleToString(glbCurrentAskBid ,0);
		createObject(14,OBJ_LABEL,0,xAxis,yAxis + glb_upper_edge_shift * 14,text);												
		
		text = "***************************************************";
		createObject(15,OBJ_LABEL,0,xAxis,yAxis + glb_upper_edge_shift * 15,text);
		
		text= "AskBid = "+intAskBid;
		createObject(16,OBJ_LABEL,0,xAxis,yAxis + glb_upper_edge_shift * 16,text);				
