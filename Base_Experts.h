...
extern color glbText_Color = clrGreenYellow;
extern int glbFont_Size = 12;
extern string dlbFont = "TimesNewRoman";
...
extern int glb_right_edge_shift = 250;
						...
						if (Table_value2[shift + i1] > x1) 
							{
							   ...
							   if (glbPrice_Bid != 0.00)
								   {
								      glbPrice_Bid = 0.00;
								      double C_Ask = SymbolInfoDouble(Symbol(),SYMBOL_ASK);
								      DataDisplay("Ask Price",glbPrice_Ask,"Bid Price",glbPrice_Bid,"Current Ask Price",C_Ask);
								      //Comment("\n","glbPrice_Ask =: ",DoubleToString(glbPrice_Ask,6),"\n","\n","glbPrice_Bid =: ",DoubleToString(glbPrice_Bid,6),"\n","\n","C_Ask =: ",DoubleToString(C_Ask,6));
								      if(blnAsk == True)
									      {        					   
										      //CloseAll();
										      //Order_Ask();
										      //DataDisplay(glbPrice_Ask,"Ask");
										      blnAsk = False;
										      blnBid = True;
									      }
								   }
						...
						if (Table_value2[shift + i1] < x2) 
							{
							   ...
							   if (glbPrice_Bid != 0.00)
								   {
								      glbPrice_Ask = 0.00;
								      double C_Bid = SymbolInfoDouble(Symbol(),SYMBOL_BID);
								      DataDisplay("Ask Price",glbPrice_Ask,"Bid Price",glbPrice_Bid,"Current Bid Price",C_Bid);								      
								      //Comment("\n","glbPrice_Bid =: ",DoubleToString(glbPrice_Bid,6),"\n","\n","glbPrice_Ask =: ",DoubleToStr(glbPrice_Ask,6),"\n","\n","C_Bid =: ",DoubleToString(C_Bid,6));
								      if(blnAsk == True)
									      {        					   
										      //CloseAll();
										      //Order_Ask();
...
void createObject(int st_ID,ENUM_OBJECT obj,int window,int xA,int yA,string txt="")
	{
		...
		ObjectSetText(glb_ea_name + "_" + IntegerToString(st_ID),txt,glbFont_Size,dlbFont,glbText_Color);
	} 
