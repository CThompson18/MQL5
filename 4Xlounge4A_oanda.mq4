   ...
   if (TimeCurrent()>last_t+TimeOut) {
      ...
      if (f1!=-1 && ff1!=-1){
         ...
         for (i=0;i<=ni;i++){
            ...
            double TP=tp[i];
            double SL=sl[i];
            int tr = TradeExists(oty[i],Magic_Number+getMagicNummber(sys[i]),getSymbol(sym[i]));
            int last_pr = LastTrade(oty[i],Magic_Number+getMagicNummber(sys[i]),getSymbol(sym[i]));
            if (tr==0) {
               Print(MathAbs(last_pr-ask));
               if (MathAbs(ask-op[i]) < Entry_Max_Offset*point*mult && (MathAbs(last_pr-ask)<(FailSafe_Margin*point*mult) || (last_pr == 0)) ){
                  if (oty[i]==OP_BUY){
                     if (OrderSend(getSymbol(sym[i]),OP_BUY,Lots,NormalizeDouble(ask,digits),Slippage,0,0,sys[i]+" - "+DoubleToStr(tk[i],0),Magic_Number+getMagicNummber(sys[i]),0,Blue)==-1) {
                           j=GetLastError();
                           Print ("TP is "+TP+"\n");
                           Print ("TP is "+SL+"\n");
                           Print (sym[i]);
                  ...
                  if (oty[i]==OP_SELL){
                     if (OrderSend(getSymbol(sym[i]),OP_SELL,Lots,NormalizeDouble(bid,digits),Slippage,0,0,sys[i]+" - "+DoubleToStr(tk[i],0),Magic_Number+getMagicNummber(sys[i]),0,Red)==-1) {
                           j=GetLastError();
                           Print ("TP is "+TP+"\n");
                           Print ("TP is "+SL+"\n");
                           Print (sym[j]);
                  ...
                  if (oty[i]==OP_BUYSTOP){
                     if (OrderSend(getSymbol(sym[i]),OP_BUYSTOP,Lots,NormalizeDouble(op[i],digits),Slippage,0,0,sys[i]+" - "+DoubleToStr(tk[i],0),Magic_Number+getMagicNummber(sys[i]),0,Blue)==-1) {
                           j=GetLastError();
                           Print ("TP is "+TP+"\n");
                           Print ("TP is "+SL+"\n");
                  ...
                  if (oty[i]==OP_BUYLIMIT){
                     if (OrderSend(getSymbol(sym[i]),OP_BUYLIMIT,Lots,NormalizeDouble(op[i],digits),Slippage,0,0,sys[i]+" - "+DoubleToStr(tk[i],0),Magic_Number+getMagicNummber(sys[i]),0,Blue)==-1) {
                           j=GetLastError();
                           if (j!=1 && j!=0) Print("BuyLimit Error :"+error(j)+" at "+getSymbol(sym[j])+" "+Period());                 
                     }
                  }
                  if (oty[i]==OP_SELLSTOP){
                     if (OrderSend(getSymbol(sym[i]),OP_SELLSTOP,Lots,NormalizeDouble(op[i],digits),Slippage,0,0,sys[i]+" - "+DoubleToStr(tk[i],0),Magic_Number+getMagicNummber(sys[i]),0,Red)==-1) {
                           j=GetLastError();
                           if (j!=1 && j!=0) Print("SellStop Error :"+error(j)+" at "+getSymbol(sym[j])+" "+Period());                 
                     }
                  }
                  if (oty[i]==OP_SELLLIMIT){
                     if (OrderSend(getSymbol(sym[i]),OP_SELLLIMIT,Lots,NormalizeDouble(op[i],digits),Slippage,0,0,sys[i]+" - "+DoubleToStr(tk[i],0),Magic_Number+getMagicNummber(sys[i]),0,Red)==-1) {
                           j=GetLastError();
            ...
            }else if (tr!=0 && TradeExistsInHistory(oty[i],Magic_Number+getMagicNummber(sys[i]),getSymbol(sym[i])) == 0) {
            ...
            if (Allow_Auto_Close){
               ...
               while (!gs){
                  ...
                  for (int t=0;t<OrdersTotal();t++){
                     ...
                     bid=MarketInfo(OrderSymbol(),MODE_BID);
                     digits = MarketInfo(OrderSymbol(),MODE_DIGITS);
                     if (OrderMagicNumber()==Magic_Number+getMagicNummber(sys[i]) && OrderType()==OP_BUY && OrderSymbol() == getSymbol(sym[i]) && !Opened(oty[i],getSymbol(sym[i]),op[i])){
                        OrderClose(OrderTicket(),OrderLots(),NormalizeDouble(bid,digits),Slippage,Pink);
                        gs=false;
                        break;
                     }
                     if (OrderMagicNumber()==Magic_Number+getMagicNummber(sys[i]) && OrderType()==OP_SELL && OrderSymbol() == getSymbol(sym[i])&& !Opened(oty[i],getSymbol(sym[i]),op[i])){
                        OrderClose(OrderTicket(),OrderLots(),NormalizeDouble(ask,digits),Slippage,Pink);
                        gs=false;
                        break;
                     }
                     if (OrderMagicNumber()==Magic_Number+getMagicNummber(sys[i]) && OrderSymbol() == getSymbol(sym[i]) && (OrderType()==OP_SELLSTOP || OrderType()==OP_SELLLIMIT || OrderType()==OP_BUYSTOP || OrderType()==OP_BUYLIMIT) && !Opened(oty[i],getSymbol(sym[i]),op[i])){
...
bool Opened(int type, string sym, double open_price){
   double point = MarketInfo(OrderSymbol(),MODE_POINT);
   int mult=1;
   if ( ((StringFind(OrderSymbol(),"JPY",0)!=-1 || StringFind(OrderSymbol(),"jpy",0)!=-1) && point<0.01) ||
        ((StringFind(OrderSymbol(),"JPY",0)==-1 || StringFind(OrderSymbol(),"jpy",0)==-1) && point<0.0001)) 
      mult=10;        
   if (OrderType() != type && OrderSymbol() == sym)
      return (false);
   if (OrderSymbol() == sym && MathAbs(OrderOpenPrice()-open_price)>Entry_Max_Offset*2*point*mult)
      return (false);
   return (true);
}


int TradeExists(int type, int m, string symbol){
   for (int i=0;i<OrdersTotal();i++){
      OrderSelect(i,SELECT_BY_POS);
      if (OrderMagicNumber()==m && OrderSymbol() == symbol && OrderType() == type ) return (OrderTicket());
   }
   for (i=0;i<OrdersHistoryTotal();i++){
      OrderSelect(i,SELECT_BY_POS,MODE_HISTORY);
      if (OrderMagicNumber()==m && OrderSymbol() == symbol && OrderType() == type)  return (-1*OrderTicket());
   }
   return (0);
}

int TradeExistsInHistory(int type, int m, string symbol){
...
double LastTrade(int type, int m, string symbol){
   datetime last = 0;
   double price = 0;
   for (int i=0;i<OrdersHistoryTotal();i++){
      OrderSelect(i,SELECT_BY_POS,MODE_HISTORY);
      if (OrderMagicNumber()==m && OrderSymbol() == symbol && OrderType() == type)  {
...
int getMagicNummber(string sys){
   if (sys == "rt") return (5001);
   if (sys == "st") return (5002);
   if (sys == "lt") return (5003);
}
