...
extern double ATR_Multiplier_Pullback = 2;
extern int ATR_Timeframe_Pullback = 0;
extern int Pullback_Timeout_minutes = 20;
...
int start()
  {
   ...
   double patr1=iATR(Symbol(),ATR_Timeframe_Pullback,ATR_Period_Pullback,Trade_Bar);

   if (pb_time != 0 && TimeCurrent()-pb_time >= 60*Pullback_Timeout_minutes)
   {
      pb_time = 0;
      pb_buy = 0;
      pb_sell = 0;    
    ...
    if (dan_up != 0){
        up_close=false;
        dn_close=true;
        pb_buy = Ask - patr1*ATR_Multiplier_Pullback;
        pb_sell = 0;
        pb_time = TimeCurrent();
    }
    if (dan_dn != 0){
        ...
        dn_close=false;
        pb_buy = 0;
        pb_sell = Bid + patr1*ATR_Multiplier_Pullback;
        pb_time = TimeCurrent();
    }
    
   if (pb_buy != 0 && Ask <= pb_buy)
   {
      up=true;
      dn=false;
   }
   if (pb_sell != 0 && Bid >= pb_sell)
   {
      up=false;
      dn=true;
   }
