...
extern int     delta=0;     // îòñòóï îò ðàñ÷åòíîãî óðîâíÿ ñòîïëîññà
extern ENUM_TIMEFRAMES TF_Tralling=0;      // òàéìôðåéì èíäèêàòîðîâ (0-òåêóùèé)

extern int     StepTrall=1;      // øàã ïåðåìåùåíèÿ ñòîïëîññ
extern int     StartTrall=1;      // ìèíèìàëüíàÿ ïðèáûëü òðàëà â ïóíêòàõ

color   text_color=clrGreen;     //öâåò âûâîäà èíôîðìàöèè

sinput string Advanced_Options="";

input int     period_ATR=14;//Ïåðèîä ATR (ìåòîä 3)

input double Step=0.02; //Parabolic Step (ìåòîä 4)
input double Maximum=0.2; //Parabolic Maximum (ìåòîä 4)
sinput  int     Magic= -1;//ñ êàêèì ìàãèêîì òðàëèòü (-1 âñå)
extern bool    GeneralNoLoss=true;   // òðàë îò òî÷êè áåçóáûòêà
extern bool    DrawArrow=false;   // ïîêàçûâàòü ìåòêè áåçóáûòêà è ñòîïîâ

input int ma_period=34;//ïåðèîä ÌÀ (ìåòîä 5)
input ENUM_MA_METHOD ma_method=MODE_SMA; // ìåòîä óñðåäíåíèÿ  (ìåòîä 5)
input ENUM_APPLIED_PRICE applied_price=PRICE_CLOSE;    // òèï öåíû  (ìåòîä 5)

input double PercetnProfit=50;//ïðîöåíò îò ïðîôèòà (ìåòîä 6)
...
int atrHandle;    // õýíäë èíäèêàòîðà Moving Average
double atrVal[];  // äèíàìè÷åñêèé ìàññèâ äëÿ õðàíåíèÿ çíà÷åíèé èíäèêàòîðà Moving Average äëÿ êàæäîãî áàðà
int sarHandle;    // õýíäë èíäèêàòîðà Moving Average
double sarVal[];  // äèíàìè÷åñêèé ìàññèâ äëÿ õðàíåíèÿ çíà÷åíèé èíäèêàòîðà Moving Average äëÿ êàæäîãî áàðà
//--------------------------------------------------------------------
int OnInit()
  {
   ...
   switch(parameters_trailing)
     {
      case 1: // ïî ýêñòðåìóìàì ñâå÷åé
         StringConcatenate(txt,"ïî ñâå÷àì ",StrPer(TF_Tralling)," +- ",delta);
         break;
      case 2: // ïî ôðàêòàëàì
         StringConcatenate(txt,"ïî ôðàêòàëàì ",StrPer(TF_Tralling)," +- ",delta);
         break;
      case 3: // ïî èíäèêàòîðó ATR
         StringConcatenate(txt,"ïî ATR (",IntegerToString(period_ATR),") ",StrPer(TF_Tralling),"+- ",delta);
         atrHandle=iATR(_Symbol,TF_Tralling,period_ATR);
         break;
      case 4: // ïî èíäèêàòîðó Parabolic
         StringConcatenate(txt,"ïî ïàðàáîëèêó (",DoubleToString(Step,2)," ",DoubleToString(Maximum,2),") ",StrPer(TF_Tralling)," +- ",delta);
         sarHandle=iSAR(_Symbol,TF_Tralling,Step,Maximum);
         break;
      case 5: // ïî èíäèêàòîðó ÌÀ
         StringConcatenate(txt,"ïî MA (",ma_period," ",ma_method," ",applied_price,") ",StrPer(TF_Tralling)," +- ",delta);
         maHandle=iMA(_Symbol,TF_Tralling,ma_period,0,ma_method,applied_price);
         break;
      case 6: // % îò ïðîôèòà
         StringConcatenate(txt," ",DoubleToString(PercetnProfit,2),"% îò ïðîôèòà)");
         break;
      default: // ïî ïóíêòàì
         StringConcatenate(txt,"ïî ïóíêòàì ",delta," ï");
         break;
     }
   if(VirtualTrailingStop)
     {
      StringConcatenate(txt,"Âèðòóàëüíûé òðàë ",txt);
     }
   else
     {
      StringConcatenate(txt,"Tðàë ",txt);
     }
   ...
   switch(parameters_trailing)
     {
      ...
      case 3: // ïî èíäèêàòîðó ATR
         ...
         if(CopyBuffer(atrHandle,0,0,3,atrVal)<0)
           {
            StringConcatenate(txt,"Îøèáêà ATR :",GetLastError());
            prc=-1;
            break;
           }
         prc=atrVal[1];
      ...
      case 4: // ïî èíäèêàòîðó Parabolic
         ...
         if(CopyBuffer(sarHandle,0,0,3,sarVal)<0)
           {
            StringConcatenate(txt,"Îøèáêà Parabolic SAR :",GetLastError());
            prc=-1;
            break;
           }
         prc=sarVal[1];
      ...
      case 5: // ïî èíäèêàòîðó ÌÀ
         ...
         if(CopyBuffer(maHandle,0,0,3,maVal)<0)
           {
            StringConcatenate(txt,"Îøèáêà Moving Average :",GetLastError());
            prc=-1;
            break;
           }
         prc=maVal[1];
